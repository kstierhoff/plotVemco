...
rm(list=ls())
# Load libraries
suppressPackageStartupMessages(library(ggplot2));library(pander);
library(lubridate); library(plyr);library(knitr);library(lubridate);
...
ab.tag.detections <- ddply(ab.res,.(DETECTEDID),summarise, N = length(X))
...
ab.res.sub <- droplevels(subset(ab.res,ab.res$HPE <= max.HPE & ab.res$DISTANCE <= max.DIST & ab.res$SPEED <= max.SPEED & ab.res$DETECTEDID %in% ab.tag.sub))
...
For starters, filtered data consisted of a subset of abalone and sync tag data that had a horizontal position error (HPE) of $\leq$ `r max.HPE`, a speed between detections of $\leq$ `r max.SPEED ` m/s, a computed distance between detections of  $\leq$ `r max.DIST` m. In addition, filtered results included only abalone tags with a minimum of `r min.DETECT` detections. This reduced the number of detections from `r nrow(ab.res)` to `r nrow(ab.res.sub)` for abalone tags (`r sprintf("%.1f",(1-(nrow(ab.res.sub)/nrow(ab.res)))*100)`% of detections removed), and `r nrow(sync.res)` to `r nrow(sync.res.sub)` for sync tags (`r sprintf("%.1f",(1-(nrow(sync.res.sub)/nrow(sync.res)))*100)`% of detections removed).
...
ggplot(ab.res.sub,aes(HPE)) + geom_histogram(binwidth=1,colour='black') +
...
ggplot() +  
  geom_polygon(data=hulls,aes(LON,LAT,group=DETECTEDID,fill=DETECTEDID),alpha = 0.5,colour='black') +
  geom_point(data=station.data,aes(LON,LAT)) +
  xlab("\nLongitude (W)") + ylab("Latitude (N)\n") + 
  theme_bw() + theme(plot.background=element_blank(),
...
ggplot(ab.res.sub,aes(x = LON,y = LAT)) +   
...
```{r MapAbaloneContoursAll,fig.height=5,fig.width=8,fig.align = 'center'}
ggplot(ab.res.sub,aes(x = LON,y = LAT)) +   
  geom_density_2d(aes(colour = DETECTEDID)) +
  geom_point(data=station.data,aes(LON,LAT)) +
  xlab("\nLongitude (W)") + ylab("Latitude (N)\n") + 
  theme_bw() + theme(plot.background=element_blank(),
                     axis.text.y = element_text(angle = 90, hjust=0.5),
